...
# ? Should I refactor to make a base class, love base classes

# TODO Still have to implement make_model
...
"""
    ...
    def __init__(self, shape, sig, psf_sig, psf_a, verbose = True, \
      ...
      psf_shape = None):
        ...
        x_pix = jnp.arange(0,shape[0])
        y_pix = jnp.arange(0,shape[1])
        X,Y = jnp.meshgrid(x_pix,y_pix, indexing = 'ij')
        self.X = X.astype(jnp.float64)
        self.Y = Y.astype(jnp.float64)

        #Get larger grid for enlarged erf_stack calculation
        x_pix_lg = jnp.arange(0,int(shape[0]*1.41)+2 )
        y_pix_lg = jnp.arange(0,int(shape[1]*1.41)+2 )
        X_lg,Y_lg = jnp.meshgrid(x_pix_lg,y_pix_lg, indexing = 'ij')
...
"""
    ...
    def get_comp_params(self, param):
        ...
        if not self.has_psf:
            ...
            final_q = jnp.array([q_in]*len(final_var))
            final_a = a_in
            final_phi = jnp.array([phi]*self.Ndof_gauss)

        else:
            if self.psf_shape == None:
                final_var = (self.var + self.psf_var[:,None]).ravel()
                final_q = jnp.sqrt( (self.var*q_in*q_in+ self.psf_var[:,None]).ravel() / (final_var) )
    ...
    def make_model(self, param):
        ...
        xc,yc,final_q,final_phi,final_a,final_var = self.get_comp_params(param)
        
        xp = (self.X[None,:,:]-xc)*jnp.cos(final_phi[:,None,None]) + (self.Y[None,:,:]-yc)*jnp.sin(final_phi[:,None,None])
        yp = -1*(self.X[None,:,:]-xc)*jnp.sin(final_phi[:,None,None]) + (self.Y[None,:,:]-yc)*jnp.cos(final_phi[:,None,None])

        r_sq = xp**2 + (yp/final_q[:,None,None])**2 
        ans =  final_a[:,None,None]/(2*jnp.pi*final_var[:,None,None])*jnp.exp( -1*r_sq / (2*final_var[:,None,None]) )
        ans = ans.sum(axis = 0)
